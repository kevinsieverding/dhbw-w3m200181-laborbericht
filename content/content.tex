...
Abschnitt \ref{sec:fazit} präsentiert abschließend ein Fazit.

\section{Use Case}\label{sec:use-case}

Das originale Setup stellt eine Reihe von Artikeln über einen Web-Server bereit, zeichnet Seitenaufrufe auf und schreibt sie in Kafka.
Von dort werden sie von einem Spark Job gelesen und aggregiert, um Popularitätsstatistiken zu erstellen, die wiederum auf der Web-Seite angezeigt werden.
...
Hierzu würde ich einen simplen Web-Shop implementieren, der dem Nutzer einen Katalog von Produkten anzeigt, welche dieser in seinen Warenkorb legen und bestellen kann.
Die Bestellungen mitsamt der Positionen würden als Nachrichten in Kafka geschrieben werden.
Ein Spark Job würde auf diesen Daten den Apriori Algorithmus anwenden, um Tupel von häufig zusammen gekauften Produkten zu identifizieren.
...
Letzenendes habe ich mich dagegen entschieden diesen Use Case zu implementieren, weil mir der notwendige Aufwand als schwierig abzuschätzen und tendenziell groß erschien.
...
Aus diesem Grund habe ich mich nach einem Use Case umgeschaut, welcher in einem kleineren Rahmen umzusetzen ist.

\subsubsection{Analyse der Parksituation in Heidelberg}

Anschließend überlegte ich die Parksituation in meinem Wohnort Heidelberg zu analysieren.
...
Während sich mit hoher Wahrscheinlichkeit ein Weg finden ließe, die API dennoch programmatisch auszulesen, so wollte ich die damit einhergehende Unsicherheit und den möglichen Aufwand für das Schreiben eines Clients/Crawlers vermeiden.
...
Hier ließen sich verschiedene, frei verfügbare Datensätze finden, die als Basis zur Implementierung einer Big Data Anwendung dienen konnten.
...
Der Big Data Stack ist im Hinblick auf die verwendeten Produkte identisch mit der ursprünglichen Anwendung.
...
Im proprietären Teil der Anwendung finden sich mehrere Instanzen einer NodeJS Web-App, welche Nutzern eine API bereitstellt.
Diese App verwendet einen Memcached als Soft State, um Ergebnisse von Datenbankabfragen für kurze Zeit zwischenzuspeichern und somit selbst bei hoher Last Anfragen mit niedrigen Latenzen bearbeiten zu können.
...
Dadurch sind die Komponenten nur gegen Kafka gekoppelt und die Datenhaltung der Web-App stellt nur ein materialisiertes Abbild der Daten in Kafka dar.
Dieses Abbild kann zum Beispiel nach einem Ausfall, oder nach Beheben eines Programmfehlers, einfach weggeworfen und neu gebildet werden.
...
Konkret könnte dies ein MQTT-Gateway sein, das es Clients erlaubt Nachrichten mit Messwerten über das bei IoT-Devices beliebte Protokoll an die Anwendung zu senden.
...
Die vorgestellte Laborarbeit wandelt die gegebene Anwendung gemäß der Aufgabenstellung ab, um einen andersartigen Big Data Use Case zu implementieren.
...
Erstens, könnte die Simulator App durch ein MQTT Gateway ersetzt werden, wodurch die Architektur den Anforderungen einer realistischeren Form des Use Case besser gerecht werden könnte.
